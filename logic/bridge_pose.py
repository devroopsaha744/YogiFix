...
class BridgePoseAngleChecker:
    """
    Angle-based logic for Bridge Pose (Setu Bandha Sarvangasana).
    Assumes a side-view perspective: The user lies on their back with knees bent, hips lifted.
    """
    ...
    def __init__(self):
        ...
        # For Bridge:
        # - Knees ~ 90° (bent, ankles under knees)
        # - Hips ~ 180° from shoulders to hips to knees if well-aligned
    ...
    def compute_pose_similarity(self, user_keypoints):
        """
        Compare user angles to the "ideal" Bridge Pose angles.
    ...
    def generate_feedback(self, overall_similarity, joint_similarities):
        """
        Provide textual feedback based on how close angles are to the ideal Bridge Pose.
        """
        feedback = []
        ...
        for joint, sim in joint_similarities.items():
            if sim < 0.7:  # <70% match for that joint
                if "knee" in joint:
                    feedback.append("Keep knees stacked over ankles, forming about 90°.")
                elif "hip" in joint:
                    feedback.append("Lift your hips higher, forming a diagonal from shoulders to knees.")
                elif "shoulder" in joint:
                    feedback.append("Keep shoulders grounded and chest open.")
        
        # If no specific feedback, but overall <80%, general comment
        if not feedback and overall_similarity < 0.8:
            feedback.append("Close! Slightly adjust your posture for a perfect Bridge Pose.")
        
        if not feedback:
            feedback.append("Great job! Your Bridge Pose looks accurate.")
...
def main():
    ...
    checker = BridgePoseAngleChecker()
    ...
    out = cv2.VideoWriter('output_Bridge.avi', fourcc, 20.0, (frame_width, frame_height))

    # Optional: make the window full-screen
    cv2.namedWindow("Bridge_Pose", cv2.WND_PROP_FULLSCREEN)
    cv2.setWindowProperty("Bridge_Pose", cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
